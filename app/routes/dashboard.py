...
HTML_BASE = """
<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>AgendaPro</title><script src="https://cdn.tailwindcss.com"></script><style>body { font-family: 'Segoe UI', sans-serif; }</style></head>
<body class="bg-slate-50">
{% with messages = get_flashed_messages(with_categories=true) %}
    {% if messages %}
    <div class="fixed top-0 right-0 m-4 z-50">{% for category, message in messages %}<div class="bg-{{ 'red' if category == 'danger' else 'green' }}-100 border-l-4 border-{{ 'red' if category == 'danger' else 'green' }}-500 text-{{ 'red' if category == 'danger' else 'green' }}-700 p-2 rounded shadow mb-2"><p>{{ message }}</p></div>{% endfor %}</div>
    {% endif %}
{% endwith %}
...
<div><p class="text-xs text-slate-400 uppercase">Panel de Control</p><h1 class="text-xl font-bold">{{ current_user.name }}</h1></div>
...
<div class="lg:w-2/3 space-y-6"><div class="bg-white rounded-xl shadow-lg p-6">
...
{% for week in current_month_days %}{% for day in week %}{% if day.month == today.month %}
<div onclick="handleDayClick('{{ day }}')" id="day-{{ day }}" class="cursor-pointer p-2 rounded text-center transition-all duration-200 text-sm font-medium {% if day < today %} text-slate-300 bg-slate-50 {% elif day in enabled_dates %} bg-green-500 text-white hover:bg-green-600 shadow-md {% else %} bg-slate-100 text-slate-700 hover:bg-slate-200 {% endif %}"><span>{{ day.day }}</span></div>
{% else %}<div class="p-2 text-slate-200"></div>{% endif %}{% endfor %}{% endfor %}</div>
<h3 class="text-center font-bold text-slate-700 mb-2 mt-8 uppercase">{{ next_month_name }}</h3>
<div class="grid grid-cols-7 gap-1 text-center text-xs font-bold text-slate-500 mb-2"><div>Dom</div><div>Lun</div><div>Mar</div><div>Mie</div><div>Jue</div><div>Vie</div><div>Sab</div></div>
<div class="grid grid-cols-7 gap-1">
{% for week in next_month_days %}{% for day in week %}{% if day.month != today.month %}
<div onclick="handleDayClick('{{ day }}')" id="day-{{ day }}" class="cursor-pointer p-2 rounded text-center transition-all duration-200 text-sm font-medium {% if day in enabled_dates %} bg-green-500 text-white hover:bg-green-600 shadow-md {% else %} bg-slate-100 text-slate-700 hover:bg-slate-200 {% endif %}"><span>{{ day.day }}</span></div>
{% else %}<div class="p-2 text-slate-200"></div>{% endif %}{% endfor %}{% endfor %}</div>
</div></div>
<div class="lg:w-1/3 space-y-6">
<div class="bg-white p-6 rounded-xl shadow-lg">
<div class="flex justify-between items-center mb-4"><h3 class="font-bold text-slate-800">Turnos de Hoy</h3><span id="today-badge" class="bg-indigo-100 text-indigo-700 text-xs font-bold px-2 py-1 rounded-full">{{ todays_appointments|length }}</span></div>
<ul id="list-today" class="space-y-3 text-sm max-h-40 overflow-y-auto">
{% for apt in todays_appointments %}<li class="flex justify-between items-center p-2 bg-slate-50 rounded"><span class="font-bold text-indigo-600">{{ apt.time.strftime('%H:%M') }}</span><span class="font-medium">{{ apt.client_name }}</span></li>{% else %}<li class="text-slate-400 text-center py-4">Sin turnos hoy.</li>{% endfor %}
</ul></div>
<div class="bg-white p-6 rounded-xl shadow-lg"><h3 class="font-bold text-slate-800 mb-4">Proximos Turnos</h3>
<ul id="list-upcoming" class="space-y-3 text-sm max-h-60 overflow-y-auto">
{% for apt in upcoming_appointments %}<li class="flex justify-between items-center p-2 bg-slate-50 rounded"><div><span class="font-bold text-slate-700">{{ apt.client_name }}</span><span class="block text-xs text-slate-500">{{ apt.date|format_date }} - {{ apt.time.strftime('%H:%M') }}</span></div></li>{% else %}<li class="text-slate-400 text-center py-4">Sin turnos futuros.</li>{% endfor %}
</ul></div>
<div class="bg-white p-6 rounded-xl shadow-lg text-center"><h3 class="font-bold text-slate-800 mb-3">Tu Link y QR</h3>
...
<img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={{ request.host_url }}agenda/{{ current_user.slug }}" alt="QR" class="mx-auto rounded"></div>
</div></div></div>
<div id="hours-modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50"><div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-sm"><h3 class="text-lg font-bold mb-4 text-slate-800">Configurar Horario</h3><p class="text-sm text-slate-600 mb-4">Fecha: <span id="modal-date" class="font-bold"></span></p>
<form id="hours-form" class="space-y-3"><div><label class="block text-sm font-medium text-slate-700">Hora Inicio</label><input type="time" id="start-time" class="w-full border p-2 rounded"></div><div><label class="block text-sm font-medium text-slate-700">Hora Fin</label><input type="time" id="end-time" class="w-full border p-2 rounded"></div>
<div class="flex gap-2 mt-4"><button type="submit" class="flex-1 bg-indigo-600 text-white py-2 rounded">Guardar</button><button type="button" onclick="hideHoursModal()" class="flex-1 bg-slate-200 py-2 rounded">Cancelar</button></div></form></div></div>
<script>
let currentEditingDate=null;function handleDayClick(d){const e=document.getElementById('day-'+d);if(e.classList.contains('bg-green-500')){showHoursModal(d);}else{toggleDay(d);}}

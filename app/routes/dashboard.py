...
def set_hours(date_str):
    ...
    try:
        ...
        # Al usar modo simple, borramos los bloques avanzados
...
def get_config(date_str):
    day = AvailableDay.query.filter_by(professional_id=current_user.id, date=date_str).first()
    data = {'start_time':'09:00', 'end_time':'18:00', 'slot_duration':30, 'has_custom': False}
    if day:
        ...
        # Verificar si tiene bloques personalizados
        if TimeBlock.query.filter_by(available_day_id=day.id).count() > 0:
...
# --- GUARDAR BLOQUES PERSONALIZADOS (EN TABLA NUEVA) ---
...
def save_custom_slots(date_str):
    ...
    # 1. Borrar bloques viejos de este día
    TimeBlock.query.filter_by(available_day_id=day.id).delete()
    ...
    # 3. Limpiar modo simple para evitar conflicto
    day.start_time = None
    day.end_time = None
